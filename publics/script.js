...
document.getElementById("sell-car-form").addEventListener("submit", function (event) {
    ...
    fetch('http://192.168.1.100:3000/submit-car-details', {
        method: 'POST',
        body: formData
    })
        .then(response => {
...
fetch('http://192.168.1.100:3000/approved-cars')
    .then(response => {
        if (!response.ok) {
        ...
        data.forEach(detail => {
            ...
            detailDiv.innerHTML = `
                ...
                <div class="car-photos">
                    ${detail.carPhotos.map(photo => `<img src="http://192.168.1.100:3000${photo}" alt="${detail.carModel}" onerror="this.onerror=null; this.src='/images/default-car.jpg'" />`).join('')}
                </div>
...
function approveCar(carName) {
    ...
    fetch('http://192.168.1.100:3000/approved-car', {
        method: 'POST',
        headers: {
